...
if variable == 'mass':
	...
	labelX = 'HNL mass'
elif variable == 'E0True':
	array = f['cascade0Dict']['Energy'][condition]
	bins = np.logspace(np.log10(1),np.log10(1e+03),31)
	labelX = 'First cascade true energy (GeV)'
elif variable == 'E1True':
	array = f['cascade1Dict']['Energy'][condition]
	bins = np.logspace(np.log10(1),np.log10(1e+03),31)
	labelX = 'Second cascade true energy (GeV)'
elif variable == 'E0':
	array = f['cascade0TaupedeDict']['Energy'][condition]
	bins = np.logspace(np.log10(1),np.log10(1e+03),31)
	labelX = 'First cascade taupede energy (GeV)'
elif variable == 'E1':
	array = f['cascade1TaupedeDict']['Energy'][condition]
	bins = np.logspace(np.log10(1),np.log10(1e+03),31)
	labelX = 'Second cascade taupede energy (GeV)'
    ...
    array = (f['millipedeDict']['E_cascade0_40m'][condition]+f['millipedeDict']['E_cascade1_40m'][condition])/f['millipedeDict']['E_tot'][condition]
    #array = (f['cascade0TaupedeDict']['Energy'][condition]+f['cascade1TaupedeDict']['Energy'][condition])/(f['retroDict']['trackEnergy'][condition]+f['retroDict']['cascadeEnergy'][condition])
    bins = np.linspace(0,1,101)
    isLogX = False
    labelX = r'Energy confinement'
elif variable == 'energyAsym':
    array = (f['millipedeDict']['E_cascade0_40m'][condition]-f['millipedeDict']['E_cascade1_40m'][condition])/(f['millipedeDict']['E_cascade0_40m'][condition]+f['millipedeDict']['E_cascade1_40m'][condition])
	...
	for i in range(np.count_nonzero(condition)):
		...
		for j in range(len(f['pulseDict']['charge'][condition_event])):
			...
			if index.size == 0:
				...
				npulse_tmp[index[0]] += 1
		npulse = npulse + npulse_tmp
		weights = weights + weights_tmp
	array = npulse
	weights = np.ones(len(array))
	...
	isLogX = False
	#isLogY = False
	labelX = 'Number of pulses'
	labelY = 'Number of DOM\'s'
elif variable == 'domXY':
