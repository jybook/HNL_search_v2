...
parser.add_option(
    ...
    "--inpath",
    dest="INPATH",
    help="Read input from INPATH (.i3{.gz/.zst} format)",
)
parser.add_option(
    "-o",
    "--outfile",
    #  default="/data/ana/BSM/HNL/MC/pickle_files/level_5",
...
parser.add_option(
    ...
    default="test_set",
    dest="NAME",
    help="Name of the pickle&log file for this run ('SETNUMBER_PROCESSING_level'",
)
parser.add_option(
    "-v",
    "--verbose",
...
parser.add_option(
    "-m", "--muon", dest="MUON", help="Use muongun specific processing (True/False)"
)
...
if options.LOGTOGILE:
    logstring = logstring = "pickle_log_{}".format(out_name)
    ...
    while infile.more():  # iterate through all physics frames end save values
        ...
        if options.MUON:
            ...
            try:
                ...
                logging.log_info(
                    "Store particle is not a MuMinus/MuPlus (as it should be)"
                )
...
store_name = "{}_reconstructed_data.pckl".format(out_name)
