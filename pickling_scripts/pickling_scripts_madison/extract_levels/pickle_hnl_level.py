...
parser.add_option("-i", "--inpath",
                 default="/data/ana/BSM/HNL/MC/190607",
                 dest="INPATH", help="Read input from INPATH subfolders (.i3{.gz/.zst} format)")
...
if options.LOGTOGILE:
    logstring = 'pickle_log_190607_level_{}'.format(options.LEVEL)
    logpath = os.path.join(options.OUTPATH, logstring)
    logging.log_info('Logpath: {}'.format(logpath))
    logging.rotating_files(logpath)


base_path = os.path.join(os.path.join(options.INPATH, options.LEVEL), 'domeff1.00')
...
for filepath in file_names:
    ...
    while(infile.more()):  # iterate through all physics frames end save values
        ...
        if(len(hnl_daughters) == 0):
            logging.log_info('Event has no (visible) decay products. Skipping this event for the moment.')
            continue
        ...
        for count_hnl_daughters, hnl_daughter in enumerate(hnl_daughters):
            ...
            if not count_hnl_daughters:
                ...
                if(np.isnan(hnl_daughter.energy)):break_bool = True                    
        ...
        # L5_WideCorridorCutTrack_L5_SPEFit11_angles_total_cos_diff.append( f['L5_WideCorridorCutTrack_L5_SPEFit11_angles']['total_cos_diff'] )
...
data_dict = {
    ...
    # 'L5_WideCorridorCutTrack_L5_SPEFit11_angles.total_cos_diff':np.array(L5_WideCorridorCutTrack_L5_SPEFit11_angles_total_cos_diff),
    }
...
store_name = 'data_190607_level_{}.pckl'.format(options.LEVEL)
